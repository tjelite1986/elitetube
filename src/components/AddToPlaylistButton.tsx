  ...
  return (
    <div className="relative">
      ...
      {open && (
        ...
        >
          <h3 className="text-sm font-semibold text-yt-text mb-3">Spara i playlist</h3>

          {loading ? (
            <p className="text-xs text-yt-muted">Laddar...</p>
          ) : (
            <div className="flex flex-col gap-1 max-h-48 overflow-y-auto mb-3">
              {playlists.length === 0 && (
                <p className="text-xs text-yt-muted">Inga playlists — skapa en nedan.</p>
              )}
              {playlists.map((pl) => (
                <button
                  key={pl.id}
          ...
          <div className="border-t border-yt-border pt-3">
            <p className="text-xs text-yt-muted mb-1.5">Skapa ny playlist</p>
